...
- The blip's distance from the origin (radar centre) is driven by the
  Velocity value:
       velocity > 0  -> blip moves CLOSER to the origin (approaching)
       velocity < 0  -> blip moves FARTHER from the origin (receding)
...
from radar_web_server import RadarWebServer
...
class UdpReceiver(QThread):
    data_received = pyqtSignal(object)   # list of (signal_dbfs, doppler_hz, velocity_ms)
    ...
    def run(self):
        ...
        while self._running:
            ...
            # Each vector = Doppler (Hz), Velocity (m/s), Signal (dBFS).
            # Forward every vector so the GUI can maintain up to five targets.
            for i in range(num_vectors):
                start = i * self.VECTOR_SIZE
                try:
                ...
                if all(math.isfinite(v) for v in
                       (doppler_hz, velocity_ms, signal_dbfs)):
                    vectors.append((signal_dbfs, doppler_hz, velocity_ms))
            ...
            if now - last_emit >= self.emit_interval:
                self.data_received.emit(vectors)
                last_emit = now
...
class RadarWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # Up to five simultaneous target tracks.
        self.MAX_TARGETS = 5
        self.targets = []

        # Target confirmation / anti-false-alarm logic.
        # A detection must be seen repeatedly before it is displayed.
        self.track_timeout = 0.75
        self.confirm_window = 0.60
        self.confirm_hits = 2

        # Association gates. A new detection inside these gates is considered
        # the same physical target instead of creating another blip.
        self.track_gate_doppler_hz = 10.0
        self.track_gate_velocity_ms = 0.6

        # Ignore very weak threshold crossings near the configured threshold.
        self.min_detection_margin_db = 3.0
    ...
    def feed_data(self, vectors):
        """Update target tracks from one or more UDP detection vectors.

        A detection is NOT displayed immediately. New tracks must be confirmed
        by repeated detections within confirm_window. This prevents isolated
        CFAR/noise peaks from becoming visible targets.
        """
        now = time.monotonic()
        ...
        self.targets = [
            ...
            if (now - t["t_last"]) <= self.track_timeout
        ]

        detections = []
        for item in vectors:
            try:
                signal_dbfs, doppler_hz, velocity_ms = map(float, item)
            except (TypeError, ValueError):
                continue

            if not all(math.isfinite(v) for v in
                       (signal_dbfs, doppler_hz, velocity_ms)):
                continue

            # Primary threshold + extra margin. A target must be meaningfully
            # above the noise threshold, not merely 0.01 dB above it.
            if signal_dbfs >= self.threshold_dbfs + self.min_detection_margin_db:
                detections.append((signal_dbfs, doppler_hz, velocity_ms))
        ...
        for signal_dbfs, doppler_hz, velocity_ms in detections:
            best_idx = None
            best_cost = float("inf")
            ...
            for idx, target in enumerate(self.targets):
                ...
                dv = abs(velocity_ms - target["velocity"])
                dd = abs(doppler_hz - target["doppler_hz"])

                # Do not associate a new detection with an old track that
                # has already been silent for too long.
                age = now - target["t_last"]
                if age > self.track_timeout:
                    continue

                if dv <= self.track_gate_velocity_ms and dd <= self.track_gate_doppler_hz:
                    cost = (
                        dv / max(self.track_gate_velocity_ms, 1e-9) +
                        dd / max(self.track_gate_doppler_hz, 1e-9)
                    )
                    if cost < best_cost:
                        best_cost = cost
                        best_idx = idx
            ...
            if best_idx is None:
                ...
                # Start an unconfirmed track. It is invisible until it has
                # accumulated enough hits inside the confirmation window.
                self.targets.append({
                ...
                used.add(len(self.targets) - 1)

            else:
                target = self.targets[best_idx]

                # If the track has been silent longer than the confirmation
                # window, restart confirmation rather than resurrecting it.
                if (now - target["t_last"]) > self.confirm_window:
                    target["hits"] = 1
                ...
                if target["hits"] >= self.confirm_hits:
                    target["confirmed"] = True

                used.add(best_idx)

        # Only confirmed targets are considered live/displayable. Unconfirmed
        # tracks are retained briefly so the next matching detection can confirm.
        if detections:
            self.last_signal, self.last_doppler, self.last_velocity = detections[0]
    ...
    def _velocity_to_radius_frac(self, velocity_ms):
        """Positive velocity = approaching/near; negative = receding/far."""
        mid = 0.5

        # Positive velocity -> smaller radius -> RADAR centre.
        # Negative velocity -> larger radius -> outer edge.
    ...
    def _tick(self):
        ...
        # Expire stale tracks independently.
        self.targets = [
            t for t in self.targets
            if (now - t["t_last"]) <= min(self.blip_lifetime, self.track_timeout)
        ]
    ...
    def _draw_target(self, painter, origin, radius_px):
        """Draw up to five live tracked targets."""
        now = time.monotonic()

        for target in self.targets:
            # Never display a one-frame/noise detection.
            if not target.get("confirmed", False):
                continue

            age = now - target["t_last"]
            life_frac = max(0.0, 1.0 - age / self.blip_lifetime)
            if life_frac <= 0:
                continue

            blink_phase = now - target["t0"]
            blink = 0.5 + 0.5 * math.sin(
                2 * math.pi * self.blink_hz * blink_phase
            )
            alpha = int(255 * life_frac * (0.35 + 0.65 * blink))
            alpha = max(0, min(255, alpha))

            r = radius_px * target["radius_frac"]
            center = self._to_point(origin, r, target["angle"])
            size = 11 + 4 * life_frac
            tri = self._triangle(center, size, target["angle"])

            painter.setPen(QPen(QColor(255, 40, 40, alpha), 1))
            painter.setBrush(QBrush(QColor(255, 30, 30, alpha)))
            painter.drawPolygon(tri)

            painter.setPen(QPen(QColor(255, 150, 150, alpha)))
            painter.setFont(QFont("Consolas", 8))
            txt = f'{target["signal"]:.1f}dB {target["velocity"]:+.2f}m/s'
    ...
    def _draw_hud(self, painter):
        ...
        count = sum(1 for t in self.targets if t.get("confirmed", False))
        color = QColor(255, 60, 60) if count else QColor(0, 255, 150)
        painter.setPen(QPen(color))
        painter.drawText(
            self.width() - 190, 18,
            f"TARGETS: {count}/{self.MAX_TARGETS}"
        )
...
class SettingsPanel(QFrame):
    def __init__(self, parent=None):
        ...
        self.track_timeout_spin = QDoubleSpinBox()
        self.track_timeout_spin.setRange(0.2, 10.0)
        self.track_timeout_spin.setValue(0.75)
        self.track_timeout_spin.setSuffix(" s")

        self.doppler_gate_spin = QDoubleSpinBox()
        self.doppler_gate_spin.setRange(0.5, 500.0)
        self.doppler_gate_spin.setValue(10.0)
        self.doppler_gate_spin.setSuffix(" Hz")

        self.velocity_gate_spin = QDoubleSpinBox()
        self.velocity_gate_spin.setRange(0.05, 20.0)
        self.velocity_gate_spin.setValue(0.6)
        self.velocity_gate_spin.setSuffix(" m/s")

        self.confirm_hits_spin = QSpinBox()
        self.confirm_hits_spin.setRange(2, 6)
        self.confirm_hits_spin.setValue(2)
        self.confirm_hits_spin.setSuffix(" hits")
        self.confirm_hits_spin.setToolTip(
            "Number of repeated detections required before a target is displayed"
        )

        self.confirm_window_spin = QDoubleSpinBox()
        self.confirm_window_spin.setRange(0.2, 2.0)
        self.confirm_window_spin.setSingleStep(0.05)
        self.confirm_window_spin.setValue(0.60)
        self.confirm_window_spin.setSuffix(" s")
        self.confirm_window_spin.setToolTip(
            "Maximum time between detections during target confirmation"
        )
        ...
        scale_form.addRow("Track timeout:", self.track_timeout_spin)
        scale_form.addRow("Doppler match gate:", self.doppler_gate_spin)
        scale_form.addRow("Velocity match gate:", self.velocity_gate_spin)
        scale_form.addRow("Confirm detections:", self.confirm_hits_spin)
        scale_form.addRow("Confirm window:", self.confirm_window_spin)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Start the LAN web dashboard.
        # It binds to 0.0.0.0 so phones on the same Wi-Fi/LAN can access it.
        self.web_server = RadarWebServer(self.radar, host="0.0.0.0", port=self.web_port)
        self.web_server.start()
        self.status_label.setText(
            f"Web RADAR: http://<this-PC-IP>:{self.web_port}"
        )
    ...
    def _apply_settings(self):
        ...
        self.radar.track_timeout = s.track_timeout_spin.value()
        self.radar.track_gate_doppler_hz = s.doppler_gate_spin.value()
        self.radar.track_gate_velocity_ms = s.velocity_gate_spin.value()
        self.radar.confirm_hits = s.confirm_hits_spin.value()
        self.radar.confirm_window = s.confirm_window_spin.value()
    ...
    # ---------------------------------------------------------------- slots
    def _on_data(self, vectors):
        self.radar.feed_data(vectors)

        if vectors:
            signal_dbfs, doppler_hz, velocity_ms = vectors[0]
            self.status_label.setText(
                f"Detections in packet: {len(vectors)} | "
                f"Strongest: {signal_dbfs:.2f} dBFS | "
